...
class ExchangeRate(Document):
	def on_submit(self):
		...
		frappe.db.sql(sql,{"exchange_rate_value":self.exchange_rate,"main_currency":main_currency,"main_currency_rate":main_currency_rate,"second_currency_rate":second_currency_rate})
		frappe.db.commit()
